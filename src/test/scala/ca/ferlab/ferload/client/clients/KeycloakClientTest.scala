...
class KeycloakClientTest extends AnyFunSuite {
  ...
  test("isTokenValid") {
    ...
    val expiredDate = Date.from(LocalDateTime.now().minusSeconds(16).atZone(ZoneId.systemDefault()).toInstant())
    val expiredToken = JWT.create()
      .withExpiresAt(expiredDate)
      .sign(algorithm);
    assert(!client.isValidToken(expiredToken))
    val validDate = Date.from(LocalDateTime.now().minusSeconds(14).atZone(ZoneId.systemDefault()).toInstant())
